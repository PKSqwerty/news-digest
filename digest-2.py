...
- SOURCE ATTRIBUTION: each headline is tagged [source]. End each bullet with source(s) in parentheses, e.g. (연합) or (BBC/Verge).
...
FORMAT — follow this template EXACTLY. Do not write paragraphs. Every story is a "•" bullet on its own line:
...
def split_sections(text):
    """Split the digest into one chunk per <섹션>; title line joins the first section."""
    lines = text.split("\n")
    chunks, current = [], ""
    def has_header(s):
        return any(l.startswith("<") for l in s.split("\n"))
    for line in lines:
        if line.startswith("<") and has_header(current):
            chunks.append(current.rstrip())
            current = ""
        current += line + "\n"
    if current.strip():
